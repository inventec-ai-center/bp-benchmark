...
def _peaks_valleys_outliers(df, args, cl_log):
	...
	df = df[~ppg_out].reset_index(drop=True)
...
def _cycle_length_limitation(df, args, cl_log):
	...
	removed_p2p = (df['ppg_p2p'] < lo_th_p2p) | (df['ppg_p2p'] > up_th_p2p) #| (df['ppg_p2p'].isna()) 

	cl_log.write(" - removed by PPG p2p distance: {} \n".format((removed_p2p).sum()))
	
	removed_vly = (df['ppg_v2v'] < lo_th_p2p) | (df['ppg_v2v'] > up_th_p2p) | (df['ppg_v2v'].isna()) 

	cl_log.write(" - removed by PPG v2v distance: {} \n".format((removed_vly).sum()))

	df = df[~removed_p2p & ~removed_vly].reset_index(drop=True)
